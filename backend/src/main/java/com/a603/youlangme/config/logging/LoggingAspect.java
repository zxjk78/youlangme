...
public class LoggingAspect {
    ...
    @AfterReturning("@annotation(com.a603.youlangme.config.logging.ExpLogging)")
    public void logExp(JoinPoint jp) throws Throwable {
        ...
        System.out.println("@@@@ ### EXP action name : "+action);
        // 원하는 Activity를 어떻게 넣어줘야 하나? 꼭 조회를 해야할까?
        // getOne을 이용? -> getReferenceById로 바뀜
        // 1.글쓰기 / 2.댓글쓰기 / 3.미팅 / 4.출석
        if (action.equalsIgnoreCase("savePost")) {

            ExpActivity activity = expActivityRepository.getReferenceById(1L);
            ...
            ExpActivity activity = expActivityRepository.getReferenceById(2L);
